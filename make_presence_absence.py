...
#read in Chemical List Presence dataset DTXSID/keyword sets
kw_sets=pd.read_csv("input/factotum_listPresence_092320_updated_chemnames_020621.csv", dtype=str)

#make reference table of DTXSID/true chemical name pairs
...
#list of keywords that, when present in a set, warrant the chemical record being dropped from the Chemical List Presence DTXSID/keyword dataset
kw_1=list(esc.loc[esc.Status==1].Keyword)
kw_1_search=list(x.split()[0] for x in kw_1)

#list of keywords that, when present in a set, warrant removal from the set, but the other keywords remain in the Chemical List Presence DTXSID/keyword dataset
...
#KW Filter 2: Remove Chemical List Presence records that contain keywords that are found in 'kw_1'
kw_sets=kw_sets.loc[kw_sets.listSets.str.contains("|".join(kw_1_search))==False]


#KW Filter 3: Remove instances of keywords found in 'kw_2', but retain the record and other keywords in the Chemical List Presence dataset
...
#Map keywords to exposure source categories and remove duplicate associations of Chemical List Presence DTXSIDS and exposure source categories
...
#make a binary dataframe showing whether any DTXSID in the Chemical List Presence dataset is (1), or is not (0) associated with an exposure source category
...
#Identify which DTXSIDs in the Chemical List Presence dataset are also in the ToxCast dataset
pres_abs["tc"]=False
pres_abs.loc[pres_abs.DTXSID.isin(toxcast_dtxsid), ["tc"]]=True


#write the presence/absence df of all Chemical List Presence DTXSIDs that map to at least two exposure, along with the there ToxCast status, to a csv.
